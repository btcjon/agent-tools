...
def _archive_files(export, *, max_files=1000, max_expanded_bytes=100_000_000):
    try:
...
def _skill_roots(files):
    ...
    if any(left in right.parents for index,left in enumerate(roots) for right in roots[index+1:]):
        raise LibraryCacheError("nested_skill_roots")
    return roots
...
class LibraryCache:
    ...
    def _verify_path(self,path,identity):
        ...
        expected={row["path"]:(row["sha256"],row["bytes"]) for row in manifest.get("files",[]) if isinstance(row,dict) and set(row)>={"path","sha256","bytes"}}
    ...
    def publish(self, exports):
        ...
        if not 1 <= len(exports) <= 5:
            raise LibraryCacheError("pilot_requires_1_to_5_exports")
        keys = [(item.kind, item.id) for item in exports]
        if len(keys) != len(set(keys)):
            raise LibraryCacheError("duplicate_export_identity")
        assembled = {}; assembled_folded=set()
        export_rows = []
        for export in sorted(exports, key=lambda item: (item.kind, item.id)):
            ...
            for name,data in sorted(files.items()):
                export_file_rows.append({"path":name,"sha256":hashlib.sha256(data).hexdigest(),"bytes":len(data)})
            ...
            for source_root in roots:
                ...
                selected = {PurePosixPath(name).relative_to(source_root).as_posix(): data for name, data in files.items()
                            if PurePosixPath(name) == source_root or source_root in PurePosixPath(name).parents}
        ...
        if not 3 <= len(assembled) <= 5:
            raise LibraryCacheError("pilot_requires_3_to_5_skills")
        file_rows = []
        for skill, files in sorted(assembled.items()):
            for relative, data in sorted(files.items()):
                file_rows.append({"path": f"{skill}/{relative}", "sha256": hashlib.sha256(data).hexdigest(), "bytes": len(data)})
        durable = {"schema_version": 1, "source": "notion-agent-skills", "exports": export_rows,
                   "skill_count": len(assembled), "files": file_rows}
